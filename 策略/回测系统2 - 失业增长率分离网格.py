...
"""
说明：增加了美国失业率指数，准备对疫情这段动手。
我们仅仅用了非常暴力的方法，左侧满做空NAU，右侧满做多AU，超出便平仓，就可以将夏普捞到差不多的位置。
现在问题是回撤很大，且在2014年初，虽然美失业率高但ratio并没有升高，导致亏损，也许可以引入中国失业率数据
"""
...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval):
    ...
    US_Unrate_ratio = np.full(length, np.nan) #美国失业率改变率
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        #制造美国失业率改变率
        US_Unrate_ratio [i] = (US_Unemployment_rate[i-1]/US_Unemployment_rate[i-31])-1 if i>30 else 0 #减去31是因为这个是月结的
        ...
        if abs(US_Unrate_ratio[i-1])>0.1: #如果失业率太高，说明纽约市场动荡明显，我们不用网格。当然这个地方可以改改，用上海的某些数据做个比对也行
            # 我们可以直接抄底，分清是左侧还是右侧
        ...
        elif US_Unemployment_rate[i-1] > 9: #太高了，清仓后就别动了，不要在这个时候开仓
            position_list[i] = 0 #清仓

        elif US_Unemployment_rate[i-2] > 5 and US_Unemployment_rate[i-1] < 5: #跌出界限，直接清仓
            position_list[i] = 0 #清仓
    ...
    data = {
        ...
        "US_Unrate_ratio":US_Unrate_ratio,
        'ratio': ratio,
...
Middle_Trade_N,Middle_postion_list,ratio,df = Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval = 0.0022)
