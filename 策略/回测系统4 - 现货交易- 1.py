...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval): #也就是说，我要在无套利区间底下做空，上面做多
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        if abs(relative_position[i-1])>0.99 or trigger == True: #我在拐点左右平仓，在另一侧好判断方向
            trigger = True
            counter += 1
            if counter >= 5:  # 如果触发0.03后已经过了两天，不需要强制观望了
                trigger = False
                position_list[i] = 0
        ...
        if ratio_value >1.002 and (abs(position_list[i-1])!= 1 ): #1.002是拿的核密度算的
            if time_judge[i] == 1:
                if ratio_value > 1 + (1.1304 / AU_price):
                    # 我们可以直接抄底，分清是左侧还是右侧
                    if expma_sign > 0.0005 and expma_sign <0.0008 and relative_position[i-1]>0.9:  # 判断为左侧升
                        position_list[i] = -1
                    elif expma_sign<-0.0005 and expma_sign >-0.0008 and relative_position[i-1]<-0.9:  # 判断为右侧降
                        position_list[i] = 1
                    else:
                ...
                if ratio_value > 1 + (1.0456 / AU_price):
                    ...
                    position_list[i] = position_list[i-1] #不交易
        elif ratio_value < 0.985 and (abs(position_list[i-1])!= 1 ): #这句话的意思就是如果有了仓位咱就一直不交易
            if ratio_value < 1 - (0.74 / AU_price + 0.49 * RMB[i] / (31.1035 * AU_price)):
                if expma_sign > 0.0005 and expma_sign<0.0008 and relative_position[i-1]<-0.9:  # 判断为左侧降
                    position_list[i] = 1
                elif expma_sign<-0.0005 and expma_sign>-0.0008 and relative_position[i-1]>0.9:  # 判断为右侧升
                    position_list[i] = -1
                else:
                    position_list[i] = position_list[i - 1]
            else:
        ...
        elif ratio_value < upper_list[i - 1] and ratio_value > lower_list[i - 1]:  # 在无套利区间里面，该平仓了
            position_list[i] = 0

        else:  #不到无套利区间，又不在特殊值外面。效果不好，不交易。或者，已经开完仓了，等等再说。
            position_list[i] = position_list[i - 1]
