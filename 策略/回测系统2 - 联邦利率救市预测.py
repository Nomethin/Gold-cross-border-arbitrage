...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval):
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        if (abs(US_Unrate_EXPMA[i-1])>7 or abs(VIX_EXPMA_change[i-1])>1) and US_Unemployment_rate[i-1]>5 or trigger==True: #两部分，一是失业率，二是恐慌指数
            if abs(Fed_FundsRate_Change[i-1])>0.03: #在较为极端的情况中，如果政府积极救市,我们观望几天再做决定
                trigger = True
                counter += 1
                if counter >=5: #如果触发0.03后已经过了五天，不需要强制观望了
                    trigger = False
                position_list[i] = 0
            else:
                # 我们可以直接抄底，分清是左侧还是右侧
