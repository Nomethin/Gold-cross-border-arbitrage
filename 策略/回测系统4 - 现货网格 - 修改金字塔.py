...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval): #也就是说，我要在无套利区间底下做空，上面做多
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        # 计算网格范围的长度
        grid_range_length = KDE_upper - KDE_lower

        # 计算网格个数（不去除余数）
        grid_count_with_remainder = int(grid_range_length / grid_interval)

        # 通过去除余数的方式调整网格范围的上限，确保网格大小可以任意取
        KDE_upper_adjusted = KDE_lower + grid_interval * grid_count_with_remainder

        # 创建网格(这边可以用adjusted但是对2伤害很大)
        grid_levels = np.arange(KDE_lower, KDE_upper, grid_interval)

        pyramid_factor = 2 * (KDE_upper-prev_ratio_value) / (KDE_upper - KDE_lower) #注意好这个数值

        if pyramid_factor > 2:
            pyramid_factor = 2

        change = p_change[i-1] * pyramid_factor * Info_dif[i-1] if i > 0 else 0 #我判断还是金字塔模型的问题
        if change > 2:
            change = 2
        elif change < -2:
            change = -2
        ...
        #下面这个KDE加上0.01即可上1.3
        if ratio_value >KDE_upper and (position_list[i-1]!= -1): #1.005是拿的核密度算的，0.978也是，是密度=0.3,加0.01是创造一个利润带
            count_avg = 0
        ...
        elif ratio_value < KDE_lower and (position_list[i-1]!= 1): #这句话的意思就是满仓位咱就一直不交易
            count_avg = 0
        ...
        #下面的这个elif就是网格的逻辑
        #向下穿买入，向上穿卖出
        elif i > 100 and (ratio_value <KDE_upper and ratio_value>KDE_lower) \
                and (abs(relative_position[i - 1]) < 1)\
                and count_avg>=10\
                and (ratio_value > upper_list[i - 1] or ratio_value < lower_list[i - 1]):
            count_avg+=1
                #if (prev_ratio_value < np.min(grid_levels)) or (prev_ratio_value > np.max(grid_levels)):  # 这个if是满足网格交易的条件,相当于从网格中穿出去了
            for k in range(len(grid_levels)-1): #小心不要和i混了，用k
                if (ratio_value>grid_levels[k] and ratio_value<grid_levels[k+1]) and prev_ratio_value<grid_levels[k]: #这个不用i-1是因为这东西不需要数据
                    position_list[i] = min(max(position_list[i - 1] - 1/len(grid_levels)*pyramid_factor*Info_dif[i-1], -1), 1)
                    break

                elif (ratio_value > grid_levels[k] and ratio_value < grid_levels[k + 1]) and prev_ratio_value >grid_levels[k]:
                    position_list[i] = max(min(position_list[i - 1] + 1/len(grid_levels)*pyramid_factor*Info_dif[i-1], 1), -1)
                    break
    ...
    return Trade_N, position_list, ratio ,df , grid_levels

Small_Trade_N,Small_postion_list,ratio,df, grid_levels = Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval = 0.0012) #这个0.0012是拿训练集算出来的
...
"""
def calculate_win_rate(Trade_N, Cumulative_Y, position_list): #只有1，0，-1的时候的胜率
    total_trades = 0
    profitable_trades = 0
...
def calculate_win_rate(Trade_N, Cumulative_Y, position_list):
    total_trades = 0
    profitable_trades = 0
    prev_position = None
    ...
    for i in range(len(Trade_N)):
        if Trade_N[i] != 0:  # 排除不交易的情况
            ...
            if prev_position is not None:  # 确保存在上一个仓位
                if (prev_position < 0 and current_position > prev_position) or (prev_position > 0 and current_position < prev_position):
                    total_trades += 1
                    if Cumulative_Y[i] >= Cumulative_Y[i - 1]:
                        profitable_trades += 1
...
def plot_graph(ratio, AU, RMB, time_judge, Cumulative_Y, Trade_N, Nega_Trade, Posi_Trade, No_Trade,trade_day,win_rate):
    """
    绘制图形函数
        ...
        No_Trade：无交易次数
    """
    trade_day = [day.strftime("%Y-%m-%d") for day in trade_day]

    # 创建一个新的图形
    fig, ax1 = plt.subplots()

    # 设置第一个坐标轴
    ax1.plot(trade_day,ratio, color='blue')
    ax1.set_xlabel('Time')
    ...
    Cumulative_Y = [0]+Cumulative_Y
    # 创建第二个坐标轴
    ax2 = ax1.twinx()
    ax2.plot(trade_day,Cumulative_Y, color='red')
    ax2.set_xlabel('Time')
    ...
    ax1.set_xlabel('Time')
    ax1.set_xlim(trade_day[0], trade_day[-1])
    ax1.set_xticks(trade_day[::90])
    ax1.set_xticklabels(trade_day[::90], rotation=45)
    ...
    table_data = [
        ['Neg. Trade', 'Pos. Trade', 'No Trade', 'Trade Frequency',"Win Rate"],
        [Nega_Trade, Posi_Trade, No_Trade, Trade_Freq, win_rate]
    ]
...
plot_graph(ratio, AU, RMB, time_judge, Cumulative_Y, Trade_N, Nega_Trade, Posi_Trade, No_Trade,trade_day,win_rate)
