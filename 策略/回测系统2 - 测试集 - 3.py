...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval):
    ...
    ZhongZheng_1000_EXPMA = functions.calculate_expma(GuoZheng_2000,period=30)
    ZhongZheng_1000_EXPMA_change = np.full(length, np.nan)
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        #制造中证1000的倾向
        ZhongZheng_1000_EXPMA_change[i] = ZhongZheng_1000_EXPMA[i - 1] - ZhongZheng_1000_EXPMA[i - 2] if i > 1 else 0
        ...
        if (abs(US_Unrate_EXPMA[i-1])>7 or abs(VIX_EXPMA_change[i-1])>1) and US_Unemployment_rate[i-1]>5 \
                ...
                or (abs(CSI_300_EXPMA_change[i-1])>18 and abs(CSI_300_EXPMA_change[i-1])<20)\
                or (abs(GuoZheng_2000_EXPMA_change[i-1])>12 and abs(GuoZheng_2000_EXPMA_change[i-1])<14)\
                or (abs(ZhongZheng_1000_EXPMA_change[i-1])>12 and abs(ZhongZheng_1000_EXPMA_change[i-1])<14): #有许多部分，待会我得重构
            if abs(Fed_FundsRate_Change[i-1])>0.01: #在较为极端的情况中，如果政府积极救市,我们观望几天再做决定
                ...
                or (abs(GuoZheng_2000_EXPMA_change[i-2])<14 and abs(GuoZheng_2000_EXPMA_change[i-1])>14)\
                or (abs(ZhongZheng_1000_EXPMA_change[i-2])<14 and abs(ZhongZheng_1000_EXPMA_change[i-1])>14): #太高了，清仓后就别动了，不要在这个时候开仓
            position_list[i] = 0 #清仓
                ...
                or (abs(GuoZheng_2000_EXPMA_change[i-2])>0.5 and abs(GuoZheng_2000_EXPMA_change[i-1])<0.5)\
                or (abs(ZhongZheng_1000_EXPMA_change[i-2])>2 and abs(ZhongZheng_1000_EXPMA_change[i-1])<2): #跌出界限，直接清仓
            position_list[i] = 0 #清仓
