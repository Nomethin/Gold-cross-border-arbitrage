...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval): #也就是说，我要在无套利区间底下做空，上面做多
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        pyramid_factor = 2 * (np.max(grid_levels)-prev_ratio_value) / (np.max(grid_levels) - np.min(grid_levels)) #这里改过了噢
        ...
        #下面这个KDE加上0.01即可上1.3
        if ratio_value >KDE_upper+0.01 and (position_list[i-1]!= -1): #1.005是拿的核密度算的，0.978也是，是密度=0.3,加0.01是创造一个利润带
            count_avg = 0
        ...
        elif ratio_value < KDE_lower-0.01 and (position_list[i-1]!= 1): #这句话的意思就是如果有了仓位咱就一直不交易
            count_avg = 0
        ...
        #下面的这个elif就是网格的逻辑
                ...
                and (abs(relative_position[i - 1]) < 0.8)\
                and count_avg>=10:
            count_avg+=1
            if (prev_ratio_value < np.min(grid_levels)) or (
                ...
                """
                下面一串是EXPMA，假设有问题，快速应对,其实不用加
                """
                if expma_trend[i-1] > 0.005:  # 通常trend 超过千分之五我们就可以说是有变化倾向（查的）
                    position_list[i] = -1  # 快速反仓：ratio涨了，即NAU相对变贵，此时应多做空NAU
                elif expma_trend[i-1] < -0.005:
                    position_list[i] = 1  # 快速反仓

                else:  # 这个else后就是正常交易时间的流程了
                    if ratio_value > 1 + 1.1304 / AU_price if time_judge[
                                                                  i] == 1 else 1 + 1.0456 / AU_price:  # i等于0时是不会交易的
                        if position_list[i - 1] + change < 1 and \
                                (position_list[i - 1] >= -1 and position_list[i - 1] <= 1):  # 然后position_list[i - 1]也不一定是正的，我们应该主要改这个
                            if change>0: #想用反方向抑制，但好像没啥用，还得看看change的源头
                    ...
                    elif ratio_value < 1 - 0.74 / AU_price - 0.49 * RMB[i] / (31.1035 * AU_price):
                        if position_list[i - 1] <= 1 and \
                                (position_list[i - 1] - change > -1 and position_list[i - 1] >= -1): #我们应该主要改这个
                            if change<0:
                                position_list[i] = min(max(position_list[i - 1] - change, -1), 1)
                            else:
                                position_list[i] = min(max(position_list[i - 1] - change, -1), 1)
