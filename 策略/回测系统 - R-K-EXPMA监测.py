...
"""
说明：在RSI-KDJ的策略基础上，在判断是否进行网格交易后，我们加入了指数移动平均值，即EXPMA。
通过计算EXPMA的变化趋势，我们可以简单的预测市场的价格趋势，在这里，依旧是拿ratio做的。当然，也许有比这个更好的。
对比了一下没有改之前的图，是在三分之二处，（ratio）市场振荡上升的时候（应该是发生了什么事情），在高斜率的情况下放弃了一些开仓操作，导致收益更高。
同时，后面的回撤也更低了。
"""
