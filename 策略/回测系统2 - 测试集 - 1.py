...
"""
说明：我们使用了OECD领先指标（一个代表着中国经济的前景和发展趋势的指标）来预测中国的经济情况。
    这个指标的处理方式和美国的失业率指标一模一样。这里用这个指标的原因，是要从中国本土的指标入手，减少偏差。
    现在这个集各种指标没有什么变化，而2019-2022三年的测试集夏普来到了0.83，卡玛比到了0.55
    （此外测试集里使用的网格大小是2014-2020这个训练集的平均上升斜率）
"""
...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval):
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        if (abs(US_Unrate_EXPMA[i-1])>7 or abs(VIX_EXPMA_change[i-1])>1) and US_Unemployment_rate[i-1]>5 \
                ...
                or (abs(CSI_300_EXPMA_change[i-1])>12 and abs(CSI_300_EXPMA_change[i-1])<20): #有许多部分，待会我得重构
            if abs(Fed_FundsRate_Change[i-1])>0.01: #在较为极端的情况中，如果政府积极救市,我们观望几天再做决定
                ...
                or (abs(CSI_300_EXPMA_change[i-2])<20 and abs(CSI_300_EXPMA_change[i-1])>20): #太高了，清仓后就别动了，不要在这个时候开仓
            position_list[i] = 0 #清仓
                ...
                or (abs(CSI_300_EXPMA_change[i-2])>1 and abs(CSI_300_EXPMA_change[i-1])<1): #跌出界限，直接清仓
            position_list[i] = 0 #清仓
