...
def Backtesting(AU, NAU, trade_day, time_judge, RMB, grid_interval): #也就是说，我要在无套利区间底下做空，上面做多
    ...
    for i, d in enumerate(trade_day):  # i是明天，只能有i-1，i-2之类的,那么一共1982项
        ...
        if (ratio[i-1]<min(0.978,lower_list[i-1]) and ratio[i-2]>max(0.978,lower_list[i-2])) \
                or (ratio[i-1]> max(1.005,upper_list[i-1]) and ratio[i-2]< min(1.005,upper_list[i-2])) \
                and position_list[i-1]== 0: #这里是检查一下，如果出去界限却没有开仓就补上
            if  (ratio[i-1]<0.978 and ratio[i-2]>0.978 and ratio[i-3]>0.978) :
                position_list[i] = 1
            elif (ratio[i-1]>1.005 and ratio[i-2]<1.005 and ratio[i-3]<1.005) :
                position_list[i] = -1
            else:
        ...
        if ratio_value >1.005 and (abs(position_list[i-1])!= 1): #1.005是拿的核密度算的，0.978也是，是密度=0.3
            count_avg = 0
            if ratio_value > (1 + (1.1304 / AU_price)) if time_judge[i] == 1 else 1 + (
                    1.0456 / AU_price):  # 向上超出无套利区间（一定要写的原因是后面KDE可能会变）(减去了重复部分)
                if i > 150 and abs(relative_position[i - 1]) > 0.995 and abs(
                        expma_sign) > 0.0003:  # 统一的150天是因为要等数据，这里的优先级比按照斜率交易高，呃呃，这一段也不交易
        ...
        elif ratio_value < 0.978 and (abs(position_list[i-1])!= 1): #这句话的意思就是如果有了仓位咱就一直不交易
            count_avg = 0
        ...
        #and (ratio_value>list_main_intervals[0] and ratio_value<list_main_intervals[1])
