...
def max_drawdown(Cumulative_Y): #输出最大回撤和回撤持续天数
    i = np.argmax((np.maximum.accumulate(Cumulative_Y) - Cumulative_Y))
    if i == 0:
        return 0
    j = np.argmax(Cumulative_Y[:i])  # 开始位置
    drawdown_max = Cumulative_Y[j] - Cumulative_Y[i]
    drawdown_rate = (Cumulative_Y[j] - Cumulative_Y[i]) / Cumulative_Y[j]
    drawdown_tian = i - j
...
# 使用高斯核函数进行核密度估计
kde = KernelDensity(kernel='gaussian').fit(ratio_array)

# 定义评估区间
eval_range = np.linspace(np.min(ratio), np.max(ratio), 1000)

# 计算每个评估点的密度分布
densities = np.exp(kde.score_samples(eval_range.reshape(-1, 1)))

# 找到主要振荡区间
threshold_density = 0.05  # 设定一个密度阈值，用于判断是否为主要振荡区间
main_intervals = []
start = None
for i, density in enumerate(densities):
    if density > threshold_density and start is None:
        start = eval_range[i]
    elif density <= threshold_density and start is not None:
        main_intervals.append((start, eval_range[i-1]))
        start = None

# 打印主要振荡区间
for interval in main_intervals:
    print("主要振荡区间：", interval)
...
"""
通过将数据分成1000个评估点，并计算每个评估点的密度分布来估计数据的振荡区间。
密度阈值是一个用户定义的参数，用于设定密度的临界值。当密度高于阈值时，我们认为该区域是主要振荡的一部分；当密度低于或等于阈值时，该区域不被视为主要振荡
较高的阈值会筛选出更密集的主要振荡区间，而较低的阈值会包含更广泛的振荡区间
"""
def find_main_oscillation_intervals(data, threshold_density=0.5):
    # 计算数据的最小值和最大值
    min_value = min(data)
    max_value = max(data)

    # 定义评估区间
    eval_range = [min_value + i * (max_value - min_value) / 1000 for i in range(1001)]

    # 计算每个评估点的密度分布
    densities = []
    for x in eval_range:
        density = sum([1 for d in data if abs(d - x) < 0.01]) / len(data)
...
for i in range(len(AU)):
    ratio[i] = (NAU[i] * RMB[i] / 31.1035) / AU[i]
main_intervals = find_main_oscillation_intervals(ratio)
#print(main_intervals)


def calculate_relative_position(AU, NAU, RMB): #
    ratio = np.full(len(AU), np.nan)
    for i in range(len(AU)):
        ratio[i] = (NAU[i] * RMB[i] / 31.1035) / AU[i]

    result = [0]  # 存储相对位置的列表，初始值为0

    for i in range(len(ratio)):
        relative_position = 0  # 初始化相对位置为0
        values = (ratio[:i-1]) # 获取0到i-1项的数据
        # 计算相对位置
        if len(values) > 0:
            avg = np.mean(values)  # 计算除了这一项的均值
            if avg < ratio[i]:
                count_smaller = sum(1 for value in values if value < ratio[i])  # 统计小于第i项的值的个数
                count_equal = sum(1 for value in values if value == ratio[i])  # 统计等于第i项的值的个数
                count_total = count_smaller + count_equal  # 总个数为小于和等于第i项的个数之和
                relative_position = count_total / len(values)  # 计算相对位置
            elif avg > ratio[i]:
                count_larger = sum(1 for value in values if value > ratio[i])  # 统计大于第i项的值的个数
                count_equal = sum(1 for value in values if value == ratio[i])  # 统计等于第i项的值的个数
                count_total = count_larger + count_equal  # 总个数为大于和等于第i项的个数之和
                relative_position = -count_total / len(values)  # 计算相对位置

        result.append(relative_position)  # 将相对位置添加到列表中

    return result  # 返回相对位置列表

result = calculate_relative_position(AU,NAU,RMB)
#print(result)
...
def Upper_Lower(AU,RMB,time_judge):
    ...
    for i in range(len(AU)):
        if time_judge[i] == 1:
            upper_list.append(1 + 1.1304 / AU[i])
            lower_list.append(1 - (0.74 / AU[i]) - (0.49 * RMB[i] / (31.1035 * AU[i])))
        else:
            upper_list.append(1 + 1.0456 / AU[i])
            lower_list.append(1 - (0.74 / AU[i]) - (0.49 * RMB[i] / (31.1035 * AU[i])))
