...
def Backtesting(AU, NAU, ratio, time_judge, RMB, p_change):
    trades = []
    positions = [0]  # 初始仓位为0，记得结尾删一个
    for i in range(1, len(ratio)):
        p_ti = 0
        change = p_change[i-1]

        if ratio[i] > 1:  # 做多AU的情况
            if time_judge[i] == 1:  # 判断是否在2020年4月9号前，即中国黄金交割费用变化前
                if ratio[i] > 1 + 0.08738 / AU[i]:  # 击穿之后，还是盈利的情况
                    position = positions[-1]
                    if position + change < 1 and position >= -1:
                        position += change
                        positions.append(position)
                        p_ti = change
                    else:  # position+change >= 1或position < -1
                        position = 0.5  # 如果条件满足，但是change超了，直接半仓
                        positions.append(0.5)
                        p_ti = 0.5 - positions[-1]
                else:
...
plt.figure(num=None, figsize=(12,6), frameon=True)
plt.title("Ratio")
plt.plot(range(len(AU)), ratio, color='blue', marker='o', linewidth=1, markersize=0.5)
#plt.show()
...
plt.figure(num=None, figsize=(12,6), frameon=True)
plt.title("Trade_N")
plt.plot(range(len(AU)-1), Trade_N, color='blue', marker='o', linewidth=1, markersize=0.5)
#plt.show()
